...
CREATE TABLE IF NOT EXISTS {schema}.{table} (
    ...
    close float,
    volume bigint,
    create_date timestamp DEFAULT GET_DATE()
);""") # PRIMARY KEY 유지를 위해 create_date 추가
...
def load(schema, table, records):
    ...
    try:
        ...
        cur.execute(f"""
                    ...
                    SELECT data, "open", high, low, close, volume FROM (
                        SELECT *, ROW_NUMBER() OVER (PARTITION BY date ORDER BY create_date DESC) AS seq
                        FROM t
                        )
                    WHERE seq = 1;
                    """) # ROW_NUMBER()를 이용하여 중복된 데이터를 제거, PRIMARY KEY 유지
